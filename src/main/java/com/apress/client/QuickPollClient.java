...
public class QuickPollClient {
	...
	public List<Poll> getAllPolls() {
		...
		ResponseEntity<List<Poll>> responseEntity = restTemplate.exchange(
				QUICK_POLL_URI_V1, HttpMethod.GET, null, responseType);

		List<Poll> allPolls = responseEntity.getBody();

		// Other possible implementations
		// List allPolls = restTemplate.getForObject(QUICK_POLL_URI_V1,
		// List.class);
		// Poll[] allPolls = restTemplate.getForObject(QUICK_POLL_URI_V1,
		// Poll[].class);
	...
	public static void main(String[] args) {
		...
		System.out.println("Updated poll has " + pollForId6.getOptions().size()
				+ " options");
